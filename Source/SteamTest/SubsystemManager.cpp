...
USubsystemManager::USubsystemManager() :
	OnCreateSessionCompleteDelegate(FOnCreateSessionCompleteDelegate::CreateUObject(this, &USubsystemManager::OnCreateSessionComplete_Internal)),
	OnFindSessionsCompleteDelegate(FOnFindSessionsCompleteDelegate::CreateUObject(this, &USubsystemManager::OnFindSessionComplete_Internal)),
	OnJoinSessionCompleteDelegate(FOnJoinSessionCompleteDelegate::CreateUObject(this, &USubsystemManager::OnJoinSessionComplete_Internal)),
	OnDestroySessionCompleteDelegate(FOnDestroySessionCompleteDelegate::CreateUObject(this, &USubsystemManager::OnDestroySessionComplete_Internal))
{
}
...
void USubsystemManager::CreateSession()
{
	...
	LastSessionSettings->Set(FName(TEXT("Identifier")), FString(TEXT("byungmeo")), EOnlineDataAdvertisementType::ViaOnlineServiceAndPing);
...
void USubsystemManager::OnCreateSessionComplete_Internal(FName SessionName, bool bWasSuccessful)
{
...
void USubsystemManager::OnFindSessionComplete_Internal(bool bWasSuccessful)
{
	...
	const FString IdentifierValue(TEXT("byungmeo"));	// TODO : 식별자 변경?
...
void USubsystemManager::JoinSession(const FOnlineSessionSearchResult& SessionResult)
{
	LOG_DARK_YELLOW(TEXT("%hs Called"), __FUNCTION__);
...
void USubsystemManager::OnJoinSessionComplete_Internal(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
void USubsystemManager::OnDestroySessionComplete_Internal(FName SessionName, bool bWasSuccessful)
{
	LOG_DARK_YELLOW(TEXT("%hs Called, SessionName %s"), __FUNCTION__, *SessionName.ToString());
...
FNamedOnlineSession* USubsystemManager::GetNamedOnlineSession(FName SessionName)
{
	const IOnlineSessionPtr SessionInterface = GetSessionInterface();
	if (SessionInterface.IsValid())
	{
...
FString USubsystemManager::GetDebugSessionInfo(FName SessionName)
{
	const IOnlineSessionPtr SessionInterface = GetSessionInterface();
	if (!SessionInterface.IsValid())
	{
...
IOnlineSessionPtr USubsystemManager::GetSessionInterface()
{
...
IOnlineFriendsPtr USubsystemManager::GetFriendsInterface()
{
	const IOnlineSubsystem* OnlineSubsystem = IOnlineSubsystem::Get();
...
void USubsystemManager::RequestInvitableFriendsList()
{
	LOG_DARK_YELLOW(TEXT("%hs Called"), __FUNCTION__);
...
void USubsystemManager::SendSessionInvite(const FUniqueNetId& FriendId)
{
	LOG_DARK_YELLOW(TEXT("%hs Called"), __FUNCTION__);
...
void USubsystemManager::PrintFriendDebugLog(const FOnlineFriend& Friend)
{
...
void USubsystemManager::OnReadFriendsComplete(int32 LocalPlayer, bool bWasSuccessful, const FString& ListName, const FString& ErrorStr) const
{
	LOG_DARK_YELLOW(TEXT("%hs Called"), __FUNCTION__);
